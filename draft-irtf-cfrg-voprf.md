...
In this section, we define three OPRF variants: a base mode, verifiable
mode, and weak verifiable mode. In the base mode, a client and server
interact to compute y = F(skS, x), where x is the client's input, skS
is the server's private key, and y is the OPRF output. The client learns
y and the server learns nothing. In the verifiable mode, the client also
gets proof that the server used skS in computing the function. Weak
verifiable mode is identical to base mode except that the client gets
a weaker proof that the server used the expected key. Absence of
certainty here means that the server can place clients into one of two
buckets.
...
The following one-byte values distinguish between these three modes:

| Mode               | Value |
|:===================|:======|
| modeBase           | 0x00  |
| modeVerifiable     | 0x01  |
| modeWeakVerifiable | 0x02  |
...
All modes of the OPRF involve an offline setup phase. In this phase,
...
The weak verifiable mode setup functions for creating client and
...
def SetupWeakVerifiableClient(suite, pkS):
  contextString = I2OSP(modeVerifiable, 1) || I2OSP(suite.ID, 2)
  return WeakVerifiableClientContext(contextString, pkS)
~~~

Note that in the weak verifiable mode, the server does not use its
public key in the computation.
...
def Unblind(blind, evaluatedElement, pkS, blindedElement, proof):
...
### WeakVerifiableClientContext {#weak-verifiable-client}

The WeakVerifiableClientContext extends the base ClientContext with the
expected server public key `pkS` with augmented `Blind()` and `Unblind()`
functions.

#### WeakVerifiableBlind

WeakVerifiableBlind uses additive and multiplicative blinding to produce
a blinded element. The output blind is composed of two Scalars, rather
than just one as in other the base and verifiable modes.
...
def Blind(input):
  ...
  blindedElement = GG.SerializeElement(R)
  blind = [r, s]

  return blind, blindedElement
~~~

#### WeakVerifiableUnblind

In this mode, `Unblind` takes three inputs. The additional inputs indicated
in {{protocol-overview}} are only omitted as they are ignored.
...
def Unblind(blind, evaluatedElement, pkS, ...):
  r = blind[0]
  s = blind[1]
  Z = GG.DeserializeElement(evaluatedElement)
