...
online phase of the protocol. Prior to this phase, keys should be
generated by calling the `KeyGen` functionality.

The `KeyGen` function takes a group `GG` and generates a private and public key
pair (skX, pkX), where skX is a random, non-zero element in the scalar
field `GG` and pkX is the product of skX and the group's fixed generator. Note
that  a pre-existing key pair can be used, and passed as a parameter
to the setup functions (in the case of the base mode, `skS` can be passed as a
parameter, and, in the case of the verifiable mode, both `skS` and `pkS` can
be passed as parameters), or the functionality can be called immediately prior
to calling the setups function.

~~~
# For the base mode
(skS, _) = KeyGen(GG)

# For the verifiable mode
(skS, pkS) = KeyGen(GG)
~~~
